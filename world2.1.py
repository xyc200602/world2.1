...
# ==================== 世界模型类 ====================
class WorldModel:
    def __init__(self, num_individuals=100000, initial_value=100, num_iterations=2000,
        ...
        num_iterations: 迭代次数 (改为2000)
        random_seed: 随机种子
        average_death_age: 平均死亡年龄（迭代次数），0表示无死亡
        inheritance_type: 继承类型 ("direct": 直接继承, "redistribute": 部分重新分配, "meritocratic": 精英继承, "equal": 完全平均分配)
    ...
    def _death_and_inheritance(self, indices):
        ...
        elif self.inheritance_type == "equal":
            # 完全平均继承：死亡人群的势力值平均分配给所有新生儿
            # 计算死亡人群的平均势力值
            avg_dead_wealth = np.mean(self.values[indices])

            # 所有新生儿都从相同的平均值开始（遗产完全重新分配）
            self.values[indices] = avg_dead_wealth

            # 重新生成能力值（无遗传性，每个人都有新的开始）
            self.ability[indices] = self._generate_ability(death_count)
    ...
    def run_simulation(self, verbose=True):
        ...
        for i in range(self.num_iterations):
            ...
            if verbose and (i + 1) % 100 == 0:  # 每100次迭代汇报一次
...
def main():
    ...
    if choice == "1":
        ...
        inheritance_type = input("请输入继承类型 (direct/redistribute/meritocratic/equal, 默认direct): ") or "direct"
        ...
        inheritance_types = ["direct", "redistribute", "meritocratic", "equal"]
