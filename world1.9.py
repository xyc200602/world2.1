...
# ==================== 世界模型类 ====================
class WorldModel:
    def __init__(self, num_individuals=100000, initial_value=100, num_iterations=400,  # 修改为400
    ...
    def plot_chart_set_1(self):
        ...
        if self.death_enabled:
            ...
            fig.suptitle(f'世界模型模拟结果 - 图表组1 (永生社会)', 
                        fontsize=16, fontweight='bold')
        ...
        ax2.set_title('2. 三种相关系数对比 (能力 vs 势力)', fontsize=12, fontweight='bold')
    ...
    def plot_chart_set_2(self):
        ...
        if self.death_enabled:
            ...
            fig.suptitle(f'世界模型模拟结果 - 图表组2 (永生社会)', 
                        fontsize=16, fontweight='bold')
        ...
        if self.death_enabled:
            ...
            # 永生社会没有死亡事件
            ax1.text(0.5, 0.5, '永生社会\n无死亡事件', 
                    horizontalalignment='center', verticalalignment='center',
                    transform=ax1.transAxes, fontsize=14)
            ax1.set_title('5. 人口动态 (永生社会)', fontsize=12, fontweight='bold')
        ...
        if self.death_enabled:
            ...
            ax2.plot(iterations, self.history['avg_age'], 'b-', linewidth=2, label='平均年龄', alpha=0.8)
            ax2.set_xlabel('迭代次数', fontsize=10)
            ax2.set_ylabel('平均年龄', fontsize=10)
            ax2.set_title('6. 人口年龄变化 (永生社会)', fontsize=12, fontweight='bold')
            ax2.grid(True, alpha=0.3)
        ...
        if self.death_enabled and self.final_stats['num_generations'] > 1:
            ...
            if len(generation_stats) > 1:
                ...
                if len(gen_numbers) > 1:
                    ...
                    # 绘制代际能力变化（使用第二个y轴）
                    ax4_ability = ax4.twinx()
                    ax4_ability.plot(gen_numbers, mean_ability, 'r-o', linewidth=2, label='平均能力值', alpha=0.8)
                    
                    ax4.set_xlabel('代数', fontsize=10)
                    ax4.set_ylabel('平均势力值 (表示个人财富，地位，声望等的一个抽象的值)', fontsize=10, color='blue')
                    ax4_ability.set_ylabel('平均能力值', fontsize=10, color='red')
                    ax4.set_title('8. 代际财富和能力传递', fontsize=12, fontweight='bold')
                    ax4.grid(True, alpha=0.3)
                    
                    # 合并图例
                    lines1, labels1 = ax4.get_legend_handles_labels()
                    lines2, labels2 = ax4_ability.get_legend_handles_labels()
            ...
            ax4.text(0.5, 0.5, '永生社会\n无代际传递', 
                    horizontalalignment='center', verticalalignment='center',
                    transform=ax4.transAxes, fontsize=12)
            ax4.set_title('8. 代际财富传递 (永生社会)', fontsize=12, fontweight='bold')
    ...
    def print_chart_explanations(self):
        ...
        if self.death_enabled and self.final_stats['num_generations'] > 1:
            print("\n8. 代际财富和能力传递:")
            print("   - 蓝线: 各代平均势力值变化 (表示个人财富，地位，声望等的一个抽象的值)")
            print("   - 红线: 各代平均能力值变化")
            print("   - 显示财富和能力的代际传递模式")
        else:
            print("\n8. 代际财富传递:")
            print("   - 永生社会没有代际传递")
